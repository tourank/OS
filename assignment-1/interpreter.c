...
void exec(char *p1, char *p2, char *p3) {

    if(strcmp(p1, p2) == 0 || strcmp(p1, p3) == 0 || strcmp(p2, p3) == 0) {

        printf("Error, one of the scripts has already been loaded");
        exit(0);
    }

    //handles filename argument verification 
    //opens each program file, calls the kernel function to load each program
    //starts the execution of all the loaded programs. does not terminate until all the programs loaded complete execution
}
